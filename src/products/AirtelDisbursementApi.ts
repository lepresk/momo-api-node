...
export class AirtelDisbursementApi {
  ...
  async getTransferStatus(externalId: string): Promise<AirtelTransaction> {
    ...
    const response = await fetch(
      `${this.baseUrl}/standard/v1/disbursements/${encodeURIComponent(externalId)}`,
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
    ...
    if (!data.data?.transaction) {
      throw new Error(`Transaction not found in Airtel system for externalId: ${externalId}`)
    }
